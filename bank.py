...
class Bankomat():
    ...
    def cnat(self, money): # не работает
    ...
    def check3(self): # переделать
        if self.quantity % 3:
            self.balance += (self.balance / 100 * 3)
...
while user_in != 3:
    if user_in == 1:
        ...
        balance.cnat(money)
        # Bankomat.take_off_percent(money) # почему класс?

    balance.quantity += 1
    # Bankomat.check3() # почему класс?
    # Bankomat.check4() # почему класс?
    print('Баланс: ', balance)  # принт неверный
    user_in = int(input('Команда:(1 - пополнить,2 - снять, 3 - выйти,'))
